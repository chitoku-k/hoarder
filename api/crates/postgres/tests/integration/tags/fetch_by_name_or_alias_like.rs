...
#[tokio::test]
async fn empty_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like([].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
...
async fn with_name_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like(["り".to_string()].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
...
#[test_context(DatabaseContext)]
#[tokio::test]
async fn with_multiple_name_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like(["り".to_string(), "あ".to_string()].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
...
async fn with_name_case_insensitive_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like(["project".to_string()].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
...
async fn with_alias_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like(["げ".to_string()].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
...
#[test_context(DatabaseContext)]
#[tokio::test]
async fn with_multiple_alias_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like(["げ".to_string(), "し".to_string()].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
...
async fn with_name_and_alias_succeeds(ctx: &DatabaseContext) {
    let repository = PostgresTagsRepository::new(ctx.pool.clone());
    let actual = repository.fetch_by_name_or_alias_like(["ん".to_string()].into_iter(), TagDepth::new(2, 2)).instrument(ctx.span.clone()).await.unwrap();
