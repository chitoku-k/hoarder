...
impl TagTypesRepository for PostgresTagTypesRepository {
    ...
    async fn create(&self, slug: &str, name: &str, kana: &str) -> Result<TagType> {
        ...
        let tag_type = match sqlx::query_as_with::<_, PostgresTagTypeRow, _>(sqlx::AssertSqlSafe(sql.as_str()), values).fetch_one(&self.pool).await {
            Ok(row) => row.into(),
            Err(sqlx::Error::Database(e)) if e.is_unique_violation() => return Err(Error::from(ErrorKind::TagTypeSlugDuplicate { slug: slug.to_string() })),
        ...
        T: Iterator<Item = TagTypeId> + Send,
    {
        ...
        let tag_types = sqlx::query_as_with::<_, PostgresTagTypeRow, _>(sqlx::AssertSqlSafe(sql.as_str()), values)
            .fetch(&self.pool)
            .map_ok(Into::into)
            .try_collect()
            .await
    ...
    async fn update_by_id(&self, id: TagTypeId, slug: Option<&str>, name: Option<&str>, kana: Option<&str>) -> Result<TagType> {
        ...
        let tag_type = match sqlx::query_as_with::<_, PostgresTagTypeRow, _>(sqlx::AssertSqlSafe(sql.as_str()), values).fetch_one(&mut *tx).await {
            Ok(row) => TagType::from(row),
            Err(sqlx::Error::RowNotFound) => return Err(Error::from(ErrorKind::TagTypeNotFound { id })),
            Err(e) => return Err(Error::other(e)),
        };
