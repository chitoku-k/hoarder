...
impl SourcesRepository for PostgresSourcesRepository {
        ...
        T: Iterator<Item = SourceId> + Send,
    {
        ...
        let sources = sqlx::query_as_with::<_, PostgresSourceExternalServiceRow, _>(sqlx::AssertSqlSafe(sql.as_str()), values)
            .fetch(&self.pool)
            .map_err(Error::other)
    ...
    async fn fetch_by_external_metadata(&self, external_service_id: ExternalServiceId, external_metadata: ExternalMetadata) -> Result<Option<Source>> {
        ...
        let source = sqlx::query_as_with::<_, PostgresSourceExternalServiceRow, _>(sqlx::AssertSqlSafe(sql.as_str()), values)
