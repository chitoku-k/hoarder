...
pub trait TagTypesRepository: Send + Sync + 'static {
    /// Creates a tag type.
    fn create(&self, slug: &str, name: &str) -> impl Future<Output = Result<TagType>> + Send;

    /// Fetches all tag types.
    fn fetch_all(&self) -> impl Future<Output = Result<Vec<TagType>>> + Send;

    /// Updates the tag type by ID.
    fn update_by_id<'a>(&self, id: TagTypeId, slug: Option<&'a str>, name: Option<&'a str>) -> impl Future<Output = Result<TagType>> + Send;
